 ...
 public:
  ...
  bool addEdge(VertexT from, VertexT to, WeightT weight) {
    if (adjacencyList.find(from) == adjacencyList.end() || adjacencyList.find(to) == adjacencyList.end()) {
      return false;  // either vertex not in graph
    }
  ...
  bool getWeight(VertexT from, VertexT to, WeightT& weight) const {
    ...
    auto const& vertex = adjacencyList.at(from);
    auto const foundEdge = vertex.find(to);
    if (foundEdge == vertex.end()) {
      return false;  // edge does not exist
    }
    weight = foundEdge->second;
    return true;
  }
  ...
  set<VertexT> neighbors(VertexT v) const {
    set<VertexT> S;
    auto const& vertex = adjacencyList.find(v);
    if (vertex != adjacencyList.end()) {
